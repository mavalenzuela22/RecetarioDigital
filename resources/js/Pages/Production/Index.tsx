...
export default function Production({ title, from, to, from_label, to_label, has_confirmed_orders, has_orders, groups, money, backUrl, productionUrl, startUrl, success }: {
    ...
    const rangeError = dates.errors.from ?? dates.errors.to;
    const acceptedRangeLabel = from === to ? from_label : `${from_label} al ${to_label}`;
    const submitDates = (event: FormEvent) => { event.preventDefault(); dates.get(productionUrl, { preserveState: true, preserveScroll: true }); };
    const startProduction = (event: FormEvent) => { event.preventDefault(); if (!dates.processing) dates.post(startUrl, { preserveScroll: true }); };
    ...
    return <AppShell title={title} back={backUrl}>
        ...
        {rangeError && <p id="production-range-error" className="error-message" role="alert"><strong>No se pudo actualizar el rango.</strong> {rangeError} La información mostrada corresponde al último rango válido: {acceptedRangeLabel}.</p>}
        <form onSubmit={submitDates} className="rounded-2xl bg-paper p-4" aria-label="Rango de producción" aria-describedby={rangeError ? 'production-range-error' : undefined}><div className="grid gap-3 sm:grid-cols-2"><Field id="from" label="Desde" error={dates.errors.from}><input id="from" name="from" type="date" value={dates.data.from} onChange={(event) => dates.setData('from', event.target.value)} aria-invalid={!!dates.errors.from} aria-describedby={dates.errors.from ? 'from-error production-range-error' : rangeError ? 'production-range-error' : undefined} /></Field><Field id="to" label="Hasta" error={dates.errors.to}><input id="to" name="to" type="date" value={dates.data.to} onChange={(event) => dates.setData('to', event.target.value)} aria-invalid={!!dates.errors.to} aria-describedby={dates.errors.to ? 'to-error production-range-error' : rangeError ? 'production-range-error' : undefined} /></Field></div><button className="button secondary mt-4 w-full" type="submit">Actualizar fechas</button></form>
